...
function App() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        ...
        alert('Ошибка при запросе данных ;c');
        console.error(error);
      }
    }
    fetchData();
  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        await axios.delete(
          `https://611545bd8f38520017a38415.mockapi.io/cart/${findItem.id}`,
        );
      } else {
  ...
  const onRemoveItem = async (id) => {
    try {
      await axios.delete(`https://611545bd8f38520017a38415.mockapi.io/cart/${id}`);
